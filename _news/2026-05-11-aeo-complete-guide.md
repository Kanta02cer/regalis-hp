...
jsonld: |
  <script type="application/ld+json">
  {
    ...
    "mainEntity": [
      ...
      {"@type":"Question","name":"トリリオンバンクではAEO対策をどのように実施していますか？","acceptedAnswer":{"@type":"Answer","text":"トリリオンバンク（トリリオンバンク）では、FAQページ・HowToスキーマ・llms.txt・JSON-LD構造化データをセットで実装するAEO統合パッケージを月額¥98,000〜（税別）で提供しています。自社サイトでの実証を経た手法を、クライアントのサイトに展開します。まずは30分のオンライン相談でお問い合わせください。"}}
    ]
  }
  </script>
...
まずは[30分のオンライン相談](https://trillion-bank.jp/trillionbank/meeting/)から。現状のAEO対応の課題やお悩みは、面談内でご相談いただけます。
...
A. トリリオンバンクでは、FAQページ・HowToスキーマ・llms.txt・JSON-LD構造化データをセットで実装するAEO統合パッケージを月額¥98,000〜（税別）で提供しています。30分のオンライン相談からお気軽にお問い合わせください。
