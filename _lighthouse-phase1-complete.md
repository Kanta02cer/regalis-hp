# Lighthouse パフォーマンス最適化 Phase 1 完了レポート
...
**最適化した画像:**
1. Hand Stitch (`/images/suits.photo/8.png`)
2. Material Archive (`/images/suits.photo/5.png`)
3. Dialogue Notes (`/images/suits.photo/4.png`)
4. NOBLE Line (`/images/suits.photo/1.png`)
5. URBAN Line (`/images/suits.photo/2.png`)
6. ROYAL Line (`/images/suits.photo/1.png`)
7. CEREMONY Line (`/images/suits.photo/3.png`)

**期待される効果:**
- 画像デコードの非同期化により、レンダリングのブロッキングを削減
- FCP（First Contentful Paint）の改善
- 約3,589 KiBの節約（Lighthouseの推奨）
...
3. **画像の非同期デコード**
   ...
   - `width`と`height`属性を設定することで、レイアウトシフトを防止（既に実装済み）
