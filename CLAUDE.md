...
## コンテンツ設計書（2026-04 確定版）

### 情報設計の基本方針

1. **「設計から始める」を全ページに通底** — 代表（Google AI Ambassador / Aalto Design Thinking）の「設計思想」を軸に
2. **3軸＋5派生の階層構造** — Core DX 3事業を主軸、残り5事業を展開事業として二階層に
3. **誠実開示を競争優位に** — 契約条件（初期6ヶ月・中途解約条件）を3箇所で明示
4. **相談導線を集約** — 全ページ末尾CTAは「DX無料相談」に一本化

### サイトマップ

```
TOP
├─ Services（Core DX 3事業）
│   ├─ AI・DX戦略コンサルティング
│   ├─ Web・システム開発
│   └─ SEO・AIOメディア運営 ★メイン事業（月額¥80,000〜）
│        └─ 料金・契約条件ページ（独立）
├─ Business（展開事業）
│   ├─ 販売促進・営業代行
│   ├─ オーダースーツ事業 Regalis
...
│   ├─ 代表タレント活動
│   └─ レザー製品（革製品）
├─ About（会社概要 / 代表メッセージ / 設計思想）
├─ Works / Case（実績）
├─ News / Insights（ブログ・お知らせ）
├─ Contact（8種相談の統合フォーム）
└─ 特定商取引法に基づく表記 ★法務必須
```
...
1. **ヒーロー** — 「設計から始める、DXカンパニー。」CTA:「無料相談する」「サービスを見る」
2. **What we do** — 3行で会社を説明。Google AI Ambassador × Aalto Design Thinking 背景
3. **Core DX Services** — 3カラム（AI・DXコンサル / Web開発 / SEO・AIOメディア）
4. **Why Regalis** — 設計から始める / 一気通貫 / 自社実証 の3要素
5. **CEO プロフィール** — 井上幹太、12年不登校→起業ストーリー
6. **実績・ケーススタディ** — 診断システム開発
7. **展開事業** — 5事業タイル
8. **ニュース・Journal**
9. **統合CTA** — 「まず、話しましょう。」
...
- 以下は2026-04-28以降、代表本人の意向により**公開済み（使用OK）**：
  ...
  - ソフトバンクアカデミア17期
  - 令和の虎 Tiger Funding 1,500万円・2連続完全ALL
...
`_news/` フォルダに以下の命名規則でMarkdownファイルを作成する：
...
title: "記事タイトル"
date: YYYY-MM-DD
category: お知らせ   # お知らせ / サービス / メディア掲載 / 採用
excerpt_text: "一覧ページに表示される短い説明文（1〜2文）"
---
```

**本文:** 通常のMarkdown記法が使える（## 見出し、**太字**、リストなど）

**URLの形式:** `/group/news/[slug]/`（slugはファイル名から日付を除いた部分）
