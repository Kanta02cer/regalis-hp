#!/usr/bin/env node
/**
 * AIO Daily Report Generator — Regalis Japan Group
 * ============================================================
 * Usage:
...
function loadConfig() {
  const defaults = {
    ...
    google: {
      ...
      gscSiteUrl:  'https://regalis-order-suits.com/',
      ga4PropertyId: null,
    },
...
async function runCitationCheck(config) {
  ...
  for (const [qi, query] of config.targetQueries.slice(0, 5).entries()) {
    ...
    try {
      ...
      results.push({
        ...
        brandMentioned: text.includes('レガリス') || text.includes('Regalis') || text.includes('RegalisJPG'),
      });
      ...
      if (e.message?.includes('429') || e.message?.includes('quota') || e.message?.includes('RESOURCE_EXHAUSTED')) {
        ...
        try {
          ...
          results.push({
            ...
            brandMentioned: retryText.includes('レガリス') || retryText.includes('Regalis'),
          });
          if (retryIsC) cited++;
          await sleep(5000);
        } catch (e2) {
  ...
  try {
    ...
    const sentQuery  = config.brandSentimentQuery ||
      `Regalis Japan Group株式会社（regalis-order-suits.com）のDX・AI検索最適化サービスについて教えてください。同社はAIO/LLMOコンサルティング・SEOメディア運営・Webシステム開発を主力事業とするIT企業です。`;
...
function renderConsole(date, config, scoreData, health, gsc, ga4, citation, sugg) {
  ...
  lines.push('');
  lines.push(col('╔══════════════════════════════════════════════════════════╗', 'cyan'));
  lines.push(col('║  ', 'cyan') + bold('AIO DAILY REPORT — Regalis Japan Group') + col('           ║', 'cyan'));
  ...
  if (gsc.status === 'ok') {
    ...
    if (gsc.reason?.includes('permission') || gsc.reason?.includes('Permission')) {
      ...
      lines.push(col('       https://search.google.com/search-console/users?resource_id=https%3A%2F%2Fregalis-order-suits.com%2F', 'yellow'));
    }
    lines.push('');
  }
...
`;
  ...
  md += `---\n\n*Generated by Regalis AIO Report Generator v1.0 — ${date}*\n`;
  return md;
}
