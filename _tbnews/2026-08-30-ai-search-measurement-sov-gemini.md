...
| 社内運用チェックリスト | 実装対象ファイル、納品基準、チェックリスト |
...
最後に、同じ質問セットで再計測します。伸びた指標、変わらなかった指標、悪化した指標を分け、次の月に実施する施策へ落とします。営業資料、販売代理店向け提案、社内運用チェックリストにも反映し、HackⅡの標準パッケージとして再利用できる状態にします。
