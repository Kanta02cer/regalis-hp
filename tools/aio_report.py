#!/usr/bin/env python3
"""
Regalis AIO Intelligence Report Generator v1.0
広告代理店・クライアント向けAIOスコア統合レポートを自動生成する。
...
def generate_report() -> dict:
    """全4軸を実行し統合レポートを生成"""
    print(f"\n{C.CYAN}{'━'*60}{C.RESET}")
    print(f"{C.BOLD}{C.WHITE}  Regalis AIO Intelligence Report v1.0{C.RESET}")
    ...
    report = {
        ...
        "site": "https://regalis-order-suits.com",
        "brand": "Regalis Japan Group",
...
def print_terminal_report(report: dict):
    ...
    print(f"\n{C.WHITE}{'═'*60}{C.RESET}")
    print(f"{C.GRAY}  Powered by Regalis AIO Intelligence{C.RESET}")
    print(f"{C.WHITE}{'═'*60}{C.RESET}\n")


def generate_markdown_report(report: dict) -> str:
    """広告代理店向けMarkdownレポートを生成"""
...
**診断日:** {now} JST | **サイト:** regalis-order-suits.com
